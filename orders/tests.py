...
class OrderTrackingAndManagementTests(TestCase):
    ...
    @patch('orders.views.create_stripe_checkout_session')
    def test_checkout_redirects_gateway_payment_to_stripe(self, mock_create_session):
        class SessionStub:
            url = 'https://checkout.stripe.com/pay/tracking-test-session'
        ...
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, 'https://checkout.stripe.com/pay/tracking-test-session')
        pedido = Pedido.objects.get()
        self.assertEqual(pedido.estado, 'pendiente_pago')
        self.assertTrue(pedido.stock_reservado)
